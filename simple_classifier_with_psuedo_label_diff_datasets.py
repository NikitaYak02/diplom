...
def train_model(
    ...
    for epoch in range(n_epochs):
        model.train()
        if epoch % 10 == 0 or epoch == epoch_for_start_pl:
        ...
        for _ in tqdm(
            ...
            if not (inputs_pl is None or labels_pl is None):
                inputs_pl, labels_pl = inputs_pl[:batch_size_train // 7], labels_pl[:batch_size_train // 7]
    ...
    if not (train_ds is None):
        train_ds.close()
        del train_data_pseudo_label, train_ds
    del model
    gc.collect()
    torch.cuda.empty_cache()
    gc.collect()
...
if __name__ == "__main__":
    ...
    list_for_dataset_path = [
        "/home/n.yakovlev/datasets/symblink/WSS2/firts_image_20",
        "/home/n.yakovlev/datasets/symblink/WSS2/firts_image_40",
        "/home/n.yakovlev/datasets/symblink/WSS2/firts_image_60",
        "/home/n.yakovlev/datasets/symblink/WSS2/firts_image_80",
        "/home/n.yakovlev/datasets/symblink/WSS2/train_valid_1",
        "/home/n.yakovlev/datasets/symblink/WSS2/train_valid_2",
        "/home/n.yakovlev/datasets/symblink/WSS2/train_valid_3",
        "/home/n.yakovlev/datasets/symblink/WSS2/train_valid_4",
        "/home/n.yakovlev/datasets/symblink/WSS2/train_valid_5"
    ]
    for _ in range(1):
        for path_to_datasets in list_for_dataset_path:
            parsed_json["path_to_datasets"] = path_to_datasets
            EXP_PATH = str(parsed_json["exprement_path"])
            writer = SummaryWriter(f'{EXP_PATH}/tensorboard_logs')
            os.makedirs(EXP_PATH, exist_ok=True)
            onlydirs = [f for f in os.listdir(EXP_PATH) if isdir(join(EXP_PATH, f))]
            cur_exp_path = f"{EXP_PATH}/{len(onlydirs)}_{path_to_datasets.split("/")[-1]}"
            ...
            criterion = nn.CrossEntropyLoss(weight=torch.tensor([1, 0.8, 1.2, 1, 1.2], device=device))
